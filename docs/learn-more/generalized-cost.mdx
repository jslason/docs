...
The difference between the two is in the interpretation. In either case, the plugins return adjusted distances (which are traversed at the specified speeds) and turning times (which are constants unaffected by speed).

The length of every edge can be scaled independently, and at intersections every left and right turn, and every movement straight through an intersection can have a distinct cost.

## LADOT Generalized Costs

Specifically in the LADOT case, we use cost supplier plugins (LaDotBikeCostSupplier, LaDotWalkCostSupplier) which use OSM tags read by LaDotCostTags.

That input OSM data must be processed into segments between intersections by scripts at (Github). This is done by the Python OSM library which naturally breaks ways into routable edges between intersections.

For the time being, elevation is reported as the percentage of each road falling into different percentage grades. Eventually we will move toward storing full elevation profiles, rather than these derived bins.
...
Tags describing intersection features (control types, crosswalks, cross_aadt) refer to the intersection at the beginning of the street segment, not the end (verify).
...
Annual average daily traffic. These are the only tags that don't have the forward and backward suffixes. Self is in the direction of the way ("forward"), cross is the traffic of the perpendicular way at the intersection.
- self_aadt
- cross_aadt
- parallel_aadt
...
not currently used but must be present:
- speed_peak
- speed_offpeak 
